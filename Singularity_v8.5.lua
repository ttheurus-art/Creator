...
local function Notify(title, content, duration)
    ...
    local contentText = tostring(content or "")
    local contentLabel = CreateText(notification, contentText, 11, Theme.SubText)
...
function TabClass:CreateInput(data)
    ...
    local label = CreateText(holder, data.Name or "Input", 13, Theme.Text)
    ...
    local box = Create("TextBox", {
        ...
        Position = UDim2.new(0.52, -8, 0.5, -16),
        Size = UDim2.new(0.48, 0, 0, 32),
        Text = tostring(data.CurrentValue or ""),
        PlaceholderText = data.PlaceholderText or "Enter value...",
        TextColor3 = Theme.Text,
...
function WindowClass:_UpdateResponsiveSize()
    ...
    local width = math.clamp(viewport.X * 0.82, 300, 520)
    local height = math.clamp(viewport.Y * 0.72, 340, 430)

    if viewport.X < 560 then
        width = math.clamp(viewport.X - 24, 296, 360)
        height = math.clamp(viewport.Y - 120, 340, 410)
    end
...
function Singularity:CreateWindow(data)
    ...
    local window = setmetatable({
        ...
        CloseMode = closeMode,
        OriginalSize = UDim2.new(0, 320, 0, 400),
    ...
    -- Execute notification: shown once when the library finishes loading.
    task.defer(function()
        Singularity:Notify("Singularity", "The Script Is Executed", 2.5)
    end)
    ...
    settings:CreateInput({
        ...
        Callback = function(value)
            ...
            if number then
                ...
                window.Main.BackgroundTransparency = number
            end
        end
    })
    ...
    executor:CreateButton({
        ...
        Callback = function()
            ...
            if ok then
                Singularity:Notify("Executor", "The Script Has Been Execute", 2.5)
            else
