...
function WindowClass:_CreateReopenButton()
    ...
    local button = Create("TextButton", {
        Parent = Gui, BackgroundColor3 = Theme.Secondary, BackgroundTransparency = 0.08,
        AnchorPoint = aerial and Vector2.new(0.5, 0.5) or Vector2.new(0.5, 0),
        Position = aerial and UDim2.new(0.5, 0, 0.5, 0) or UDim2.new(0.5, 0, 0, 10),
        Size = aerial and UDim2.new(0, 150, 0, 44) or UDim2.new(0, 230, 0, 42),
        Text = aerial and "Singularity" or "Singularity  •  Open", TextColor3 = Theme.Text,
        TextSize = 12, Font = Enum.Font.GothamBold, Visible = false, AutoButtonColor = false, ZIndex = 50
    })
    Corner(button, 14); Stroke(button, Theme.Stroke, 1)
    button.Activated:Connect(function() self:Open() end)
    if aerial then MakeDraggable(button, button, {Enabled = true}) end
...
function Singularity:TabWindow(name)
    local host = CurrentWindow
    if not host or host.Destroyed then
        return nil
    end

    local title = tostring(name or "Window")
    local tab = host:CreateTab({Name = title})

    -- TabWindow is a long rounded window/card like Settings.
    tab.Button.Size = UDim2.new(1, -12, 0, 54)
    tab.Button.BackgroundColor3 = Theme.Secondary
    tab.Button.BackgroundTransparency = 0.08
    Corner(tab.Button, 14)
