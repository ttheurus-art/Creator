...
function WindowClass:_UpdateResponsiveSize()
    ...
    if viewport.X < 720 then
        ...
        height = math.max(290, math.floor(width * (450 / 700)))
    end
...
function WindowClass:_CreateReopenButton()
    local aerial = self.CloseMode == "Fold"
    local button = Create("TextButton", {
...
function Singularity:CreateWindow(data)
    ...
    local closeMode = rawget(_G, "WindowCloseMode") or "Top Fold"
    if closeMode ~= "Top Fold" and closeMode ~= "Fold" then
        closeMode = "Top Fold"
    end
    ...
    local window = setmetatable({
        ...
        CloseMode = closeMode,
        Transparency = 0.10,
        OriginalSize = UDim2.new(0, 700, 0, 450),
