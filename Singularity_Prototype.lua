--[[
    Singularity UI Library v8.5.0
    Original UI library
    Theme: Blue Glass / Singularity V8.5
    Mobile + PC focused
]]
...
function WindowClass:CreateTab(data)
    ...
    text.Position = UDim2.new(0, data.Icon and 43 or 18, 0, 0)
    text.Size = UDim2.new(1, data.Icon and -55 or -32, 1, 0)
...
function WindowClass:_UpdateResponsiveSize()
    ...
    local width = math.clamp(viewport.X * 0.82, 320, 700)
    local height = math.clamp(width * (5 / 7.6), 290, 460)

    if viewport.X < 560 then
        width = math.max(300, viewport.X - 24)
        height = math.clamp(width * 0.66, 280, 400)
    end

    self.OriginalSize = UDim2.new(0, width, 0, height)

    -- Keep the navigation usable on narrow phone screens.
    if viewport.X < 560 and self.Sidebar and self.Pages then
        local sidebarWidth = math.clamp(math.floor(width * 0.31), 108, 145)
        self.Sidebar.Size = UDim2.new(0, sidebarWidth, 1, -58)
        self.Pages.Position = UDim2.new(0, sidebarWidth, 0, 58)
        self.Pages.Size = UDim2.new(1, -sidebarWidth, 1, -58)
    elseif self.Sidebar and self.Pages then
        self.Sidebar.Size = UDim2.new(0, 175, 1, -58)
        self.Pages.Position = UDim2.new(0, 175, 0, 58)
        self.Pages.Size = UDim2.new(1, -175, 1, -58)
    end
...
function WindowClass:_AnimateMain(show)
    if show then
        ...
        self.Main.Size = UDim2.new(0, 8, 0, 8)
        self.Main.BackgroundTransparency = 1

        local tween = Tween(self.Main, 0.32, {
        ...
        if tween then
            tween.Completed:Connect(function()
                if self.Main and self.Main.Parent and self.Opened then
                    self.Main.Size = self.OriginalSize
        ...
        self:_Center()
        local tween = Tween(self.Main, 0.24, {
...
function WindowClass:_CreateReopenButton()
    local aerial = self.CloseMode == "Aerial"
    local button = Create("TextButton", {
        Parent = Gui, BackgroundColor3 = Theme.Secondary, BackgroundTransparency = 0.06,
        AnchorPoint = Vector2.new(0.5, 0),
        Position = UDim2.new(0.5, 0, 0, -48),
        Size = UDim2.new(0, 150, 0, 42),
        Text = "💠", TextColor3 = Theme.Text,
        TextSize = 22, Font = Enum.Font.GothamBold, Visible = false, AutoButtonColor = false, ZIndex = 50
    })
    Corner(button, 15); Stroke(button, Theme.Stroke, 1)
        ...
        if self.ReopenButton then
            task.delay(0.25, function()
                if self.ReopenButton and not self.Opened then
                    ...
                    self.ReopenButton.Position = UDim2.new(0.5, 0, 0, -48)
                    Tween(self.ReopenButton, 0.24, {
...
function WindowClass:Open()
    ...
    if self.ReopenButton then
        Tween(self.ReopenButton, 0.16, {
            Position = UDim2.new(0.5, 0, 0, -48)
        }, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
        task.delay(0.17, function()
            if self.ReopenButton and self.Opened then
                self.ReopenButton.Visible = false
            end
        end)
    end
...
function Singularity:TabWindow(name)
    -- TabWindow is a convenience API for the CURRENT window.
    -- It must never create/destroy another Window.
    local window = CurrentWindow
    if not window or window.Destroyed then
        return nil
    end

    local title = tostring(name or "Tab")
    ...
    function proxy:TabButton(n, callback)
        return tab:CreateButton({Name = tostring(n or "Button"), Callback = callback})
    end

    function proxy:TabToggle(n, callback)
        return tab:CreateToggle({Name = tostring(n or "Toggle"), CurrentValue = false, Callback = callback})
    end

    function proxy:TabDropdown(n, options, callback)
        return tab:CreateDropdown({Name = tostring(n or "Dropdown"), Options = options or {}, Callback = callback})
    end

    function proxy:TabSlider(n, min, max, callback)
        return tab:CreateSlider({Name = tostring(n or "Slider"), Range = {min or 0, max or 100}, CurrentValue = min or 0, Callback = callback})
    end

    function proxy:TabText(n, text)
        return tab:CreateParagraph({Title = tostring(n or "Text"), Content = tostring(text or "")})
    end

    function proxy:CreateSection(text)
        return tab:CreateSection(text)
    end

    function proxy:CreateButton(data)
        return tab:CreateButton(data)
    end

    function proxy:CreateToggle(data)
        return tab:CreateToggle(data)
    end

    function proxy:CreateParagraph(data)
...
function Singularity:CreateWindow(data)
    ...
    local closeMode = data.CloseMode or "Top Fold"
    if closeMode ~= "Top Fold" and closeMode ~= "Aerial" then
        closeMode = "Top Fold"
    end
    ...
    local main = Create("Frame", {
        Parent = Gui,
        BackgroundColor3 = Theme.Background,
        BackgroundTransparency = math.clamp(tonumber(data.Transparency) or 0.10, 0, 1),
    ...
    local window = setmetatable({
        ...
        CloseMode = closeMode,
        Transparency = tonumber(data.Transparency) or 0.10,
    ...
    local transparencyInput = settings:CreateTextBox({
        Name = "Transparency",
        Height = 72,
        Text = tostring(data.Transparency or 0.10),
        PlaceholderText = "0.0 - 1.0",
        TextSize = 13
    })
    transparencyInput.TextBox.FocusLost:Connect(function()
        local value = tonumber(transparencyInput:Get())
        if value then
            value = math.clamp(value, 0, 1)
            main.BackgroundTransparency = value
            transparencyInput:Set(string.format("%.2f", value))
        end
    end)
    ...
    settings:CreateDropdown({
        Name = "Close Mode",
        Options = {"Top Fold", "Aerial"},
        CurrentOption = window.CloseMode,
        Callback = function(option)
            if option == "Top Fold" or option == "Aerial" then
                window.CloseMode = option
                if window.ReopenButton then
                    window.ReopenButton:Destroy()
                    window.ReopenButton = nil
                    window:_CreateReopenButton()
                    if not window.Opened then window.ReopenButton.Visible = true end
                end
            end
        end
    })
    ...
    executor:CreateButton({
        ...
        Callback = function()
            ...
            if ok then
                Singularity:Notify("Executor", "The Script Has Been Execute", 2)
            else
    ...
    Singularity:Notify("Singularity", "The Script Is Executed", 2)
