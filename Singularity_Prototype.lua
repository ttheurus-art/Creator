...
local ExecutionNotificationShown = false
local Gui = nil
...
local function Notify(title, content, duration)
    ...
    contentLabel.Size = UDim2.new(1, -25, 0, 0)
    contentLabel.TextWrapped = true
    contentLabel.Visible = (content or "") ~= ""
...
function WindowClass:CreateTab(data)
    ...
    assert(self.TabButtons and self.TabButtons.Parent, "Singularity: TabButtons is unavailable")
    assert(self.Pages and self.Pages.Parent, "Singularity: Pages is unavailable")

    local tabName = tostring(data.Name or "Tab")
    ...
    if data.Icon then
        local icon = CreateText(tabButton, "●", 9, Theme.Muted)
    ...
    text.Position = UDim2.new(0, data.Icon and 40 or 16, 0, 0)
    text.Size = UDim2.new(1, data.Icon and -50 or -28, 1, 0)
    text.ZIndex = 5
...
function WindowClass:SelectTab(tab)
    ...
    for _, other in ipairs(self.Tabs) do
        ...
        other.Button.BackgroundColor3 = active and Theme.Tertiary or Theme.Secondary
        other.Text.TextColor3 = active and Theme.Text or Theme.SubText
    end
...
function WindowClass:_UpdateResponsiveSize()
    ...
    local width = math.clamp(viewport.X * 0.62, 240, 500)
    local height = math.clamp(width * 0.70, 230, 350)

    if viewport.X < 560 then
        width = math.max(230, viewport.X - 60)
        height = math.clamp(width * 0.72, 220, 300)
    end
...
function Singularity:TabWindow(name)
    ...
    assert(window and not window.Destroyed, "Singularity:CreateWindow must be called before TabWindow")
    ...
    assert(tab, "Singularity: failed to create tab")
    ...
    function proxy:TabDropdown(n, options, callback)
        return tab:CreateDropdown({Name = tostring(n or "Dropdown"), Options = options or {}, CurrentOption = (options and options[1]) or nil, Callback = callback})
    end

    function proxy:TabColorpicker(n, callback, defaultColor)
        return tab:CreateColorPicker({Name = tostring(n or "Color"), Color = defaultColor or Color3.fromRGB(100, 170, 255), Callback = callback})
    end

    function proxy:TabSlider(n, min, max, callback)
        return tab:CreateSlider({Name = tostring(n or "Slider"), Range = {min or 0, max or 100}, Increment = 1, Suffix = "", CurrentValue = min or 0, Callback = callback})
    end

    function proxy:TabText(n, content)
        return tab:CreateParagraph({Title = tostring(n or "Text"), Content = tostring(content or "")})
    end
...
function Singularity:CreateWindow(data)
    ...
    local closeMode = data.CloseMode or WindowCloseMode or "Top Fold"
    ...
    local title = CreateText(topbar, data.Name or "Singularity V1", 15, Theme.Text)
    ...
    local window = setmetatable({
        ...
        OriginalSize = UDim2.new(0, 520, 0, 360),
        DraggableEnabled = true,
        TopFoldEnabled = (closeMode == "Top Fold"),
        Folded = false
    }, WindowClass)
    ...
    minimize.Activated:Connect(function()
        if window.TopFoldEnabled then
            ...
            minimize.Text = window.Folded and "+" or "-"
        else
            window:Minimize()
        end
    end)
    ...
    if not ExecutionNotificationShown then
        ExecutionNotificationShown = true
        Singularity:Notify("The Library Is Executed", "", 5)
    end
