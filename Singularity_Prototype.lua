...
function WindowClass:CreateTab(data)
    ...
    -- Tabs are placed directly in the sidebar with explicit positions.
    -- This avoids UIListLayout/ScrollingFrame layout issues that could leave
    -- a created tab invisible even though the tab object exists.
    local tabButton = Create("TextButton", {
        Parent = self.TabButtons,
        BackgroundColor3 = Theme.Secondary,
        BackgroundTransparency = 0.08,
        Position = UDim2.new(0, 0, 0, (index - 1) * 59),
    ...
    -- Keep the button's own Text as the actual tab label so it cannot be
    -- hidden by a child-label sizing issue.
    tabButton.Text = (data.Icon and "   " or "") .. tabName
...
function WindowClass:SelectTab(tab)
    if not tab or self.Destroyed then
        return
    end
    ...
    for _, other in ipairs(self.Tabs) do
        ...
        if active then
            other.Button.BackgroundColor3 = Theme.Tertiary
            other.Button.TextColor3 = Theme.Text
        else
            other.Button.BackgroundColor3 = Theme.Secondary
...
function Singularity:TabWindow(name)
    ...
    -- TabWindow is a tab API, not a second Window. Reuse the current window.
