...
-- Rayfield-style convenience API
-- Supports both the native Create* API and short Tab* helpers.
-- Examples:
-- local Window = Singularity:CreateWindow({Name = "My Window"})
-- local Tab = Window:CreateTab({Name = "Test"})
-- Tab:CreateButton({Name = "Click Test", Callback = function() end})
-- Tab:TabButton("Click Test", function() end)
-- Tab:TabButton("Click Test")(function() end)

function TabClass:TabButton(name, callback)
    local title = tostring(name or "Button")
    if callback == nil then
        return function(cb)
            return self:CreateButton({Name = title, Callback = cb})
        end
    end
    return self:CreateButton({Name = title, Callback = callback})
end

function TabClass:TabToggle(name, callback)
    local title = tostring(name or "Toggle")
    if callback == nil then
        return function(cb)
            return self:CreateToggle({Name = title, CurrentValue = false, Callback = cb})
        end
    end
    return self:CreateToggle({Name = title, CurrentValue = false, Callback = callback})
end

function TabClass:TabDropdown(name, options, callback)
    local title = tostring(name or "Dropdown")
    options = options or {}
    if callback == nil then
        return function(cb)
...
function TabClass:TabColorpicker(name, callback, defaultColor)
    local title = tostring(name or "Color")
    if callback == nil then
        return function(cb)
...
function TabClass:TabSlider(name, min, max, callback)
    local title = tostring(name or "Slider")
    min = min or 0
    max = max or 100
    if callback == nil then
        return function(cb)
...
function TabClass:TabText(name, text)
...
function WindowClass:WindowTab(name)
...
function WindowClass:TabWindow(name)
    return self:WindowTab(name)
end
...
function Singularity:TabWindow(name)
    ...
    if not window or window.Destroyed then
        window = self:CreateWindow({Name = "Singularity", CloseMode = "Top Fold"})
    end

    local tab = window:WindowTab(name)
    window.IsTabWindow = true
    return tab
end

function Singularity:WindowTab(name)
    local window = CurrentWindow
    if not window or window.Destroyed then
        return nil
    end
    return window:WindowTab(name)
end
