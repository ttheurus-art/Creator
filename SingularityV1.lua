...
function WindowClass:_UpdateResponsiveSize()
    ...
    local width = math.clamp(viewport.X * 0.72, 280, 600)
    local height = math.clamp(width * 0.76, 270, 430)

    if viewport.X < 560 then
        width = math.max(270, viewport.X - 40)
        height = math.clamp(width * 0.78, 250, 360)
    end
...
function Singularity:TabWindow(name)
    local window = self:CreateWindow({Name = tostring(name or "Singularity"), CloseMode = "Top Fold"})
...
function Singularity:CreateWindow(data)
    ...
    local window = setmetatable({
        ...
        OriginalSize = UDim2.new(0, 600, 0, 430),
