...
local Theme = {
    ...
    Tertiary = Color3.fromRGB(24, 20, 36),
    Hover = Color3.fromRGB(31, 26, 46),
...
function WindowClass:_UpdateResponsiveSize()
    ...
    local isSmall = viewport.X < 700 or viewport.Y < 500

    local width
    local height

    if isSmall then
        width = math.floor(math.max(330, viewport.X * 0.92))
        height = math.floor(math.max(280, viewport.Y * 0.78))
    else
        width = math.min(720, math.floor(viewport.X * 0.78))
        height = math.min(500, math.floor(viewport.Y * 0.72))
    end
    ...
    if self.Opened and not self.Minimized then
        self.Main.Size = self.Folded and UDim2.new(0, width, 0, 58) or self.OriginalSize
        self:_Center()
    end
end
...
function Singularity:CreateWindow(data)
    ...
    local title = CreateText(topbar, data.Name or "Singularity", 15, Theme.Text)
    ...
    local window = setmetatable({
        ...
        CloseMode = closeMode,
        OriginalSize = UDim2.new(0, 640, 0, 440),
    ...
    settings:CreateToggle({
        ...
        CurrentValue = true,
        Callback = function(value)
            window.DraggableEnabled = value
            if window.DragState then
                window.DragState.Enabled = value
            end
        end
    })
    ...
    settings:CreateToggle({
        ...
        CurrentValue = false,
        Callback = function(value)
            window.TopFoldEnabled = value
            if not value and window.Folded then
    ...
    settings:CreateParagraph({
        Title = "Top Fold",
        Content = "When enabled, tap the window title area to fold the interface into the top bar."
    })

  
    -- Small title-area control for Top Fold, keeping the normal drag area intact.
